...
if (!empty($_POST))
{
	...
	$sql = "INSERT INTO $mytable (firstname, lastname, email, bookingref) VALUES (?,?,?,?)";
	?>
		<div class="report">Using SQL to save form: <?php echo $sql ?></div>
		<?php
	$stmt = mysqli_prepare($dbhandle, $sql );
	mysqli_stmt_bind_param($stmt, 'ssss', $webdata['firstname'], $webdata['lastname'], $webdata['email'], $webdata['bookingref']);
