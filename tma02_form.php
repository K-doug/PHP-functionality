...
if( !empty($webdata['editid'])){
	$sql = "SELECT id, firstname, lastname , email, bookingref FROM $mytable WHERE id = ?" ;
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"  name = "tt284guestform<?php echo $row["id"]; ?>" onsubmit="return validate()">
	...
	<span id="fbfirstname">  </span><?php if (!$valid){echo $formerror["firstname"]  ;} ?>
	...
	<span id="fblastname">  </span><?php if (!$valid){echo $formerror["lastname"]  ;} ?></p>
	
	<p><label for="email">Email: </label>
	<input type="text" name="email" id="email"   maxlength="50" minlength="4" value="<?php if ($valid){echo htmlspecialchars($row["email"]);}
	else{echo htmlspecialchars($webdata["email"]);}?>" onchange="fieldcheck(this.id)">
	<span id="fbemail">  </span><?php if (!$valid){echo  $formerror["email"]  ;} ?></p>
	
	<p><label for="bookingref">Booking Reference: </label>
	<input type="text" name="bookingref" id="bookingref" required maxlength="8" minlength="8" value="<?php if ($valid){echo htmlspecialchars($row["bookingref"]);}
	else{echo htmlspecialchars($webdata["bookingref"]);} ?>" onchange="fieldcheck(this.id)">
	<span id="fbbookingref"> </span><?php if (!$valid){echo $formerror["bookingref"]  ;} ?></p>
