...
function fieldcheck(id){  //check if the field represented by the unique identifier id is valid.
    ...
    var failmessage;                                         // variable to hold message should test fail
   ...
   //test for email
	...
	//test for bookingref
    if (id == "bookingref"){                             // Allow valid booking numbers
		pattern = /^[A-Z]{3}-\d{4}$/ ;                      // test the value against an appropriate regular expression 
        failmessage="Format required: 3 letters - 4 numbers";
	}
